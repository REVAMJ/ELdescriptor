function desc = EL65(patches)
    %  prepeared for HPatches 
    desc = DDDescriptor(patches);
end

function GDdescriptor = DDDescriptor(patches)
...
for k=1:numPatches
    patch=patches(:,:,:,k);
 %    patch = imfilter(patches(:,:,:,k), wi, 'replicate', 'same','conv');
    %image filtering
    [GDmaps,Angle] = GDerivMaps(patch,GDfilters); 
