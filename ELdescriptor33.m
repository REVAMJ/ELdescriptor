function [GDdescriptor]=ELdescriptor33(patches)
...
[meje1,~]=computePoolingCenters (r1,8,33,warea1,pi/8);
[meje2,centri2]=computePoolingCenters (r2,8,33,warea2,0);

w2=cell(8,1);
norM=imfilter(ones(33,33),warea2);
for i=1:8
...
%centers
for i=1:n
    centri(i,1) = c+ round(r*cos(2*pi/n*(i-1)+ang)+0.5); %x
    centri(i,2) = c- round(r*sin(2*pi/n*(i-1)+ang)+0.5); %y

    pCenters(i,1)=(centri(i,1)); %x
    pCenters(i,2)=(centri(i,2)); %y
end
...
%first circle
for i=1:8    
    w=w1(m1(i,7):m1(i,8),m1(i,5):m1(i,6));   
      %first derivative 
    for k=1:8 
...
%first derivative 
G1x = imfilter(im, filtri{1}, 'replicate', 'same');
G1y = imfilter(im, filtri{2}, 'replicate', 'same');
kot(:,:,1)=atan2(G1y,G1x);
A(:,:,1)=cos(kot(:,:,1)).*G1x+sin(kot(:,:,1)).*G1y;

%second derivative
G2_0 = imfilter(im, filtri{3}, 'replicate', 'same');
G2_60 = imfilter(im, filtri{4}, 'replicate', 'same');
G2_120 = imfilter(im, filtri{5}, 'replicate', 'same');
...
%ww=sqrt(sum(sum(g2_0.*g2_0)));
ww=1;
filtri{3}=g2_0/ww;

g2_60=imrotate(g2_0, 60, 'bilinear');
%ww=sqrt(sum(sum(g2_60.*g2_60)));
ww=1;
filtri{4}=g2_60/ww;

g2_120=imrotate(g2_0, 120, 'bilinear');
%ww=sqrt(sum(sum(g2_120.*g2_120)));
ww=1;
filtri{5}=g2_120/ww;
end
%%
